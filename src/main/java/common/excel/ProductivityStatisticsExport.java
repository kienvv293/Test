 ...
 * @author phamxuantung
 */
public class ProductivityStatisticsExport extends AbstractReportExport {
...
outer:		for(int i=0;i<rowNum;i++) {
	...
	private void setCellData(Row row, Row newRow, Map<String, Object> listInfo) {
		...
		while (itCell.hasNext()) {
			...
			if (StringCommon.isNull(valueStr)) {
				continue;
			}
...
//				setCellValue(newCell,getDetailValue(listInfo,keyStr,formatStr));
				...
				if (ProductivityStatistics.MEMBER_TYPE_CODER.equals(formatStr) && "1".equals(listInfo.get("memberType"))
						|| ProductivityStatistics.MEMBER_TYPE_TESTER.equals(formatStr) && "2".equals(listInfo.get("memberType"))) {
					newCell.setCellValue(listInfo.get(keyStr).toString());
				}
